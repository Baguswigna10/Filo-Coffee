    ...
    <form action="{{ $product->exists ? route('admin.products.update', $product) : route('admin.products.store') }}"
        ...
        <div class="admin-card p-6 space-y-5">
            ...
            <div class="flex items-center gap-3 pb-4 border-b border-white/[0.06]">
                <div class="w-8 h-8 bg-mocca/10 rounded-lg flex items-center justify-center ring-1 ring-mocca/20">
                    <svg class="w-4 h-4 text-mocca" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4"/></svg>
                </div>
                <span class="text-cream/50 text-sm font-medium">Detail Produk</span>
            </div>

            <div class="grid grid-cols-2 gap-4">
                <div class="col-span-2">
                    <label class="block text-xs text-cream/40 mb-2 font-medium uppercase tracking-wider">Nama Produk *</label>
                    <input type="text" name="name" value="{{ old('name', $product->name) }}" required class="input-field">
                    @error('name')<span class="text-red-400 text-xs mt-1 block">{{ $message }}</span>@enderror
                </div>
                <div>
                    <label class="block text-xs text-cream/40 mb-2 font-medium uppercase tracking-wider">Asal Daerah *</label>
                    <input type="text" name="origin" value="{{ old('origin', $product->origin) }}" required class="input-field" placeholder="Aceh Gayo, Flores, dll.">
                    @error('origin')<span class="text-red-400 text-xs mt-1 block">{{ $message }}</span>@enderror
                </div>
                <div>
                    <label class="block text-xs text-cream/40 mb-2 font-medium uppercase tracking-wider">Roast Level *</label>
                ...
                <div class="col-span-2">
                    <label class="block text-xs text-cream/40 mb-2 font-medium uppercase tracking-wider">Flavor Notes *</label>
                    ...
                    @error('flavor_notes')<span class="text-red-400 text-xs mt-1 block">{{ $message }}</span>@enderror
                </div>
            </div>
        </div>
        ...
        <div class="admin-card p-6 space-y-5">
            <div class="flex items-center gap-3 pb-4 border-b border-white/[0.06]">
                ...
                <span class="text-cream/50 text-sm font-medium">Harga & Stok</span>
            </div>
            ...
            <div class="grid grid-cols-3 gap-4">
                ...
                <div>
                    <label class="block text-xs text-cream/40 mb-2 font-medium uppercase tracking-wider">Berat (gram) *</label>
                    <input type="number" name="weight_grams" value="{{ old('weight_grams', $product->weight_grams ?? 250) }}" required min="0" class="input-field">
                </div>
                <div>
                    <label class="block text-xs text-cream/40 mb-2 font-medium uppercase tracking-wider">Stok *</label>
        ...
        <div class="admin-card p-6 space-y-5">
            <div class="flex items-center gap-3 pb-4 border-b border-white/[0.06]">
                ...
                <span class="text-cream/50 text-sm font-medium">Deskripsi & Media</span>
            </div>
            ...
            <div>
                <label class="block text-xs text-cream/40 mb-2 font-medium uppercase tracking-wider">Foto Produk</label>
                @if($product->image)
                <div class="mb-3 relative inline-block group">
                    <img src="{{ $product->image_url }}" class="w-28 h-28 object-cover rounded-xl ring-1 ring-white/[0.06] group-hover:ring-mocca/30 transition-all duration-300">
                </div>
                @endif
            ...
            <div class="flex gap-6 pt-2">
                <label class="flex items-center gap-2.5 cursor-pointer group">
                    ...
                    <input type="checkbox" name="is_active" value="1" {{ old('is_active', $product->is_active ?? true) ? 'checked' : '' }} class="w-4 h-4 accent-[#CCB196] rounded cursor-pointer">
                    <span class="text-sm text-cream/50 group-hover:text-cream/70 transition-colors">Aktif dijual</span>
                </label>
                <label class="flex items-center gap-2.5 cursor-pointer group">
                    <input type="hidden" name="is_featured" value="0">
                    <input type="checkbox" name="is_featured" value="1" {{ old('is_featured', $product->is_featured) ? 'checked' : '' }} class="w-4 h-4 accent-[#CCB196] rounded cursor-pointer">
