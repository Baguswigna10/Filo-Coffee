            ...
            <tr>
                <td>
                    <div class="flex items-center gap-3">
                        <div class="w-10 h-10 flex-shrink-0 bg-olive-50 rounded-xl overflow-hidden ring-1 ring-olive-900/5">
                            @if($product->image)
                                <img src="{{ $product->image_url }}" class="w-full h-full object-cover">
                            @else
                                <div class="w-full h-full flex items-center justify-center text-olive-900/20 text-sm">🫘</div>
                            @endif
                        </div>
                        <div>
                            <p class="text-olive-900 font-medium text-sm">{{ $product->name }}</p>
                            <p class="text-olive-900/30 text-xs font-semibold">{{ $product->weight_grams }}gr</p>
                        </div>
                    </div>
                </td>
                <td class="text-olive-900/60 font-medium">{{ $product->origin }}</td>
                <td>
                    <span class="text-olive-850/60 text-xs font-semibold bg-olive-50 px-2.5 py-1 rounded-lg">{{ $product->roast_level }}</span>
                </td>
                <td class="text-olive-900 font-bold">{{ $product->formatted_price }}</td>
                <td>
                    @if($product->stock == 0)
                    <span class="inline-flex items-center gap-1.5 text-red-600 font-bold text-xs">
                    ...
                    @else
                    <span class="text-olive-900/60 font-semibold text-sm">{{ $product->stock }}</span>
                    @endif
                </td>
                <td class="text-right">
                    <div class="flex items-center justify-end gap-1">
                        <a href="{{ route('admin.products.edit', $product) }}" class="w-8 h-8 flex items-center justify-center rounded-lg text-olive-900/30 hover:text-mocca-dark hover:bg-mocca/10 transition-all duration-200">
                        ...
                        <form action="{{ route('admin.products.destroy', $product) }}" method="POST" onsubmit="return confirm('Hapus produk ini?')">
                            @csrf @method('DELETE')
                            <button type="submit" class="w-8 h-8 flex items-center justify-center rounded-lg text-olive-900/20 hover:text-red-650 hover:bg-red-50 transition-all duration-200">
                ...
                <td colspan="6" class="!py-16 text-center">
                    <div class="flex flex-col items-center gap-3">
                        <div class="w-12 h-12 bg-olive-50 rounded-xl flex items-center justify-center">
                            <svg class="w-6 h-6 text-olive-900/20" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4"/></svg>
                        </div>
                        <p class="text-olive-900/30 text-sm">Belum ada produk.</p>
                    </div>
                </td>
            </tr>
            @endforelse
