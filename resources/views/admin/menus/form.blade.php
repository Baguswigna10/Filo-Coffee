    ...
    <form action="{{ $menu->exists ? route('admin.menus.update', $menu) : route('admin.menus.store') }}"
        ...
        <div class="admin-card p-6 space-y-5">
            ...
            <div class="flex items-center gap-3 pb-4 border-b border-olive-900/5">
                ...
                <span class="text-olive-900/60 text-sm font-medium">Detail Menu</span>
            </div>

            <div>
                <label class="block text-[10px] text-olive-900/40 mb-2 font-bold uppercase tracking-wider">Nama Menu *</label>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    <label class="block text-[10px] text-olive-900/40 mb-2 font-bold uppercase tracking-wider">Kategori *</label>
                ...
                <div>
                    ...
                    @error('price')<span class="text-red-650 text-xs mt-1 block">{{ $message }}</span>@enderror
                </div>
            </div>
            ...
            <div>
                <label class="block text-[10px] text-olive-900/40 mb-2 font-bold uppercase tracking-wider">Foto Menu</label>
                @if($menu->image)
                <div class="mb-3 relative inline-block group">
                    <img src="{{ $menu->image_url }}" class="w-28 h-28 object-cover rounded-xl ring-1 ring-olive-900/5 group-hover:ring-mocca/30 transition-all duration-300">
                </div>
                @endif
                ...
                @error('image')<span class="text-red-650 text-xs mt-1 block">{{ $message }}</span>@enderror
            </div>

            <div class="grid grid-cols-2 gap-4">
                <div>
                    <label class="block text-[10px] text-olive-900/40 mb-2 font-bold uppercase tracking-wider">Sort Order</label>
            ...
            <div class="flex gap-6 pt-2">
                <label class="flex items-center gap-2.5 cursor-pointer group">
                    ...
                    <input type="checkbox" name="is_available" value="1" {{ old('is_available', $menu->is_available ?? true) ? 'checked' : '' }}
                           class="w-4 h-4 accent-mocca text-olive-600 border-olive-900/10 rounded cursor-pointer">
                    <span class="text-sm text-olive-900/50 group-hover:text-olive-900 transition-colors font-medium">Tersedia</span>
                </label>
                <label class="flex items-center gap-2.5 cursor-pointer group">
